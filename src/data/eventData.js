//Blog images
import { blog } from '../assets/images';
const { blog2, blog4, blog5, blog8 } = blog;

//Event data
export const eventData = [
    {
        id: 1,
        title: 'WAVE SURFING',
        image: blog2,
        location: 'Olympia Exhibition Centre, London, United Kingdom',
        time: '8am',
        date: {
            day: '20',
            month: 'SEP',
        },
        text_content_1:
            'Interdum et malesuada fames ac ante ipsum primis in faucibus. Curabitur purus nisi, pretium id malesuada sit amet, sagittis quis ligula. Phasellus at urna purus. Quisque finibus nisi ac tortor luctus accumsan. Suspendisse aliquam turpis vitae magna auctor gravida. Vestibulum placerat purus vitae enim vehicula, quis vehicula felis mollis. Pellentesque nec pretium sapien.',

        text_content_2:
            'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla non odio quis enim ornare elementum. Sed placerat lectus ac venenatis porttitor. Nam libero ex, lacinia id fermentum id, faucibus eget ligula. In vel sapien facilisis, tincidunt lectus convallis, feugiat dolor.',
        category: ['sport'],
        tag: ['fishing', 'sport'],
    },
    {
        id: 2,
        title: 'SPECIALITY & FINE FOOD FAIR 2015',
        image: blog5,
        location: 'Alexandra Palace , London, United Kingdom',
        time: '8pm',
        date: {
            day: '31',
            month: 'DEC',
        },
        text_content_1:
            'Interdum et malesuada fames ac ante ipsum primis in faucibus. Curabitur purus nisi, pretium id malesuada sit amet, sagittis quis ligula. Phasellus at urna purus. Quisque finibus nisi ac tortor luctus accumsan. Suspendisse aliquam turpis vitae magna auctor gravida. Vestibulum placerat purus vitae enim vehicula, quis vehicula felis mollis. Pellentesque nec pretium sapien.',

        text_content_2:
            'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla non odio quis enim ornare elementum. Sed placerat lectus ac venenatis porttitor. Nam libero ex, lacinia id fermentum id, faucibus eget ligula. In vel sapien facilisis, tincidunt lectus convallis, feugiat dolor.',
        category: ['food'],
        tag: ['food', 'park'],
    },
    {
        id: 3,
        title: 'LONDON EDGE 2015',
        image: blog8,
        location: 'Business Design Centre, London, United Kingdom',
        time: '9am',
        date: {
            day: '05',
            month: 'NOV',
        },
        text_content_1:
            'Interdum et malesuada fames ac ante ipsum primis in faucibus. Curabitur purus nisi, pretium id malesuada sit amet, sagittis quis ligula. Phasellus at urna purus. Quisque finibus nisi ac tortor luctus accumsan. Suspendisse aliquam turpis vitae magna auctor gravida. Vestibulum placerat purus vitae enim vehicula, quis vehicula felis mollis. Pellentesque nec pretium sapien.',

        text_content_2:
            'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla non odio quis enim ornare elementum. Sed placerat lectus ac venenatis porttitor. Nam libero ex, lacinia id fermentum id, faucibus eget ligula. In vel sapien facilisis, tincidunt lectus convallis, feugiat dolor.',
        category: ['hiking'],
        tag: ['camp', 'hiking'],
    },
    {
        id: 3,
        title: 'SKY SPORTS GOLF',
        image: blog4,
        location: 'London, London, United Kingdom',
        time: '2pm',
        date: {
            day: '15',
            month: 'JUN',
        },
        text_content_1:
            'Interdum et malesuada fames ac ante ipsum primis in faucibus. Curabitur purus nisi, pretium id malesuada sit amet, sagittis quis ligula. Phasellus at urna purus. Quisque finibus nisi ac tortor luctus accumsan. Suspendisse aliquam turpis vitae magna auctor gravida. Vestibulum placerat purus vitae enim vehicula, quis vehicula felis mollis. Pellentesque nec pretium sapien.',

        text_content_2:
            'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla non odio quis enim ornare elementum. Sed placerat lectus ac venenatis porttitor. Nam libero ex, lacinia id fermentum id, faucibus eget ligula. In vel sapien facilisis, tincidunt lectus convallis, feugiat dolor.',
        category: ['golf'],
        tag: ['golf', 'sport'],
    },
];
